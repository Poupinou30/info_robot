...
int w_plot(float x[], float y[], float angle[], float distance[], int length) {//x c'est angle
  ...
  if(verbose) fprintf(gnuplot,"set xlabel 'x'\n");
  if(verbose) fprintf(gnuplot,"set ylabel 'y'\n");
  if(verbose) fprintf(gnuplot, "plot '-' using 1:2 with points lc rgb 'blue', \'-' using 1:2 with points lc rgb 'orange', \'-' using 1:2 with points lc rgb 'red' \n");
  //if(verbose) fprintf(gnuplot, "plot 'balises' using 1:2 with points lc rgb 'green' \n");
  
  for (int i = 0; i < 3; ++i) {
    //if(y[i]*cos(x[i]*(M_PI/180.0))<3.0 && y[i]*cos(x[i]*(M_PI/180.0)>-3.0))
    if(verbose) fprintf(gnuplot, "%f %f \n", distance[i]*cos(-angle[i]*(M_PI/180.0)), distance[i]*sin(-angle[i]*(M_PI/180.0)));
    //if(verbose) fprintf(gnuplot, "%d %d\n", x[i], y[i]);
  }
  if(verbose) fprintf(gnuplot,  "e\n");
  //if(verbose) fprintf(gnuplot, "replot 'data' using 1:2 with points lc rgb 'blue' \n");
  ...
  if(verbose) fprintf(gnuplot,  "e\n");
  //if(verbose) fprintf(gnuplot, "replot 'robot' using 1:2 with points lc rgb 'red'\n");
  double r=0.1;
  double x0,y0;
  double y1,x1;
  for(y1=y0-r;y1<=y0+r;y1=y1+0.1){
    x1=sqrt(r*r-(y1-y0)*(y1-y0))+x0; 
    if(verbose) fprintf(gnuplot,"%lf\t %lf l\n",x1,y1);
  }
  for(y1=y0+r;y1>=y0-r;y1=y1-0.1){
    x1=-sqrt(r*r-(y1-y0)*(y1-y0))+x0; 
    if(verbose) fprintf(gnuplot,"%lf\t %lf l\n",x1,y1);
  ...
  if(verbose) fprintf(gnuplot, " %f %f \n",0.0,0.0);
  if(verbose) fprintf(gnuplot, "e\n");
  //if(verbose) fprintf(stdout, "Click Ctrl+d to quit...\n");
  fflush(gnuplot);
  getchar();
    ...
    */
    ...
    while(distance(beaconTab[0].angle, beaconTab[1].angle,beaconTab[0].distance, beaconTab[1].distance) < distance(beaconTab[0].angle, beaconTab[2].angle,beaconTab[0].distance, beaconTab[2].distance) || distance(beaconTab[1].angle, beaconTab[2].angle,beaconTab[1].distance, beaconTab[2].distance)<distance(beaconTab[0].angle, beaconTab[2].angle,beaconTab[0].distance, beaconTab[2].distance)){
		...
		//ici j'ai juste rajouté ces conditions là
                ...
                float myX = 0, myY = 0;
        if(verbose) fprintf(stderr,"wtfq\n");
		/*for (int j = 0; j < 3; i++) {
                ...
                myX /= 3.0;
                myY /= 3.0;
                        if(verbose) fprintf(stderr,"myX = %f myY = %f \n",myX,myY);

            //FIN TEST AUGUSTIN*/
        if(verbose) fprintf(stderr,"wtfq7\n");
