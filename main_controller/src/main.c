...
int main(){//VRAI MAIN STRATEGY UTILE
    ...
    while (1)
    {
        ...
        if (currentElapsedTime >= timeDelay)
        {
            ...
            if(myMoveType != GRABBING_MOVE&&((pow(filteredSpeedX *filteredSpeedX + filteredSpeedY*filteredSpeedY,0.5) < 0.4 && fabs(filteredSpeedOmega)<20) /*||fabs(*myPos.theta - *myOdometryPos.theta)> 5*/ )&& lidarElapsedTime < 100){
                resetOdometry();
