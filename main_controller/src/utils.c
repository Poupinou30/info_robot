...
endZone* computeBestStealZone(){
    ...
    if(smallestDistance == INFINITY){
        printf("found no zones to steal");
        return NULL;
    } 
...
solarZone* computeBestSolarZone(){
    ...
    if(myTeamColor == 0 ){
        if(solarZones[0].stateCenter != 0){
            bestSolarZone = &solarZones[0];
            printf("solarZone %d: state = %d \n", 0,solarZones[0].stateCenter);
        }
    }else{
        if(myTeamColor && solarZones[2].stateCenter != myTeamColor){
            bestSolarZone = &solarZones[2];
            printf("solarZone %d: state = %d \n", 2, solarZones[2].stateCenter);
        }
    } 
    printf("BestSolarZone found: zoneID = %d, state = %d \n\n",bestSolarZone->zoneID, bestSolarZone->stateCenter);
